...
static double less_than(double a, double b) {return a < b ? 1.0 : 0.0;}
static double less_than_or_equal(double a, double b) {return a <= b ? 1.0 : 0.0;}
static double greater_than(double a, double b) {return a > b ? 1.0 : 0.0;}
static double greater_than_or_equal(double a, double b) {return a >= b ? 1.0 : 0.0;}
static double equal_to(double a, double b) {return a == b ? 1.0 : 0.0;}
static double not_equal_to(double a, double b) {return a != b ? 1.0 : 0.0;}
    ...
    do {
        ...
        if ((s->next[0] >= '0' && s->next[0] <= '9') || s->next[0] == '.') {
            ...
            if (isalpha(s->next[0])) {
                ...
                s->type = TOK_INFIX;
                s->function = less_than_or_equal;
                s->next += 2;
            } else if (s->next[0] == '>' && s->next[1] == '=') {
                s->type = TOK_INFIX;
                s->function = greater_than_or_equal;
                s->next += 2;
            } else if (s->next[0] == '=' && s->next[1] == '=') {
                s->type = TOK_INFIX;
                s->function = equal_to;
                ...
                switch (s->next++[0]) {
                    ...
                    case '<': s->type = TOK_INFIX; s->function = less_than; break;
                    case '>': s->type = TOK_INFIX; s->function = greater_than; break;
...
static te_expr *equality_expr(state *s);
static te_expr *comparison_expr(state *s);
...
static te_expr *comparison_expr(state *s) {
    ...
    while (s->type == TOK_INFIX &&
           (s->function == less_than || s->function == less_than_or_equal ||
            s->function == greater_than || s->function == greater_than_or_equal)) {
        te_fun2 t = s->function;
        next_token(s);
        te_expr *right = sum_expr(s);
        CHECK_NULL(right, te_free(ret));

        te_expr *prev = ret;
        ret = NEW_EXPR(TE_FUNCTION2 | TE_FLAG_PURE, ret, right);
        CHECK_NULL(ret, te_free(right), te_free(prev));
...
static te_expr *equality_expr(state *s) {
    te_expr *ret = comparison_expr(s);
    CHECK_NULL(ret);

    while (s->type == TOK_INFIX && (s->function == equal_to || s->function == not_equal_to)) {
        te_fun2 t = s->function;
        next_token(s);
        te_expr *right = comparison_expr(s);
        CHECK_NULL(right, te_free(ret));
...
static te_expr *logical_and_expr(state *s) {
    te_expr *ret = equality_expr(s);
    CHECK_NULL(ret);

    while (s->type == TOK_INFIX && s->function == logical_and) {
        te_fun2 t = s->function;
        next_token(s);
        te_expr *right = equality_expr(s);
        CHECK_NULL(right, te_free(ret));
...
static te_expr *expr(state *s) {
    ...
    while (s->type == TOK_INFIX && s->function == logical_or) {
        te_fun2 t = s->function;
        next_token(s);
        te_expr *right = logical_and_expr(s);
        CHECK_NULL(right, te_free(ret));
