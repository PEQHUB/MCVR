...
void TextureArrayManager::flushUploads(std::shared_ptr<vk::VMA> vma,
                                        std::shared_ptr<vk::Device> device,
    ...
    for (auto& upload : stagedUploads_) {
        ...
        // Create staging buffer for this layer — kept alive in inflightStagingBuffers_
        // until GPU finishes executing the copy commands
...
std::vector<uint32_t> TextureArrayManager::flushAndMipgenDirtyLayers(
    uint32_t arrayId,
    std::shared_ptr<vk::VMA> vma,
    std::shared_ptr<vk::Device> device,
    std::shared_ptr<vk::CommandBuffer> cmdBuffer) {

    // Collect which layers of this array have pending uploads
    std::vector<uint32_t> dirtyLayers;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        for (auto& upload : stagedUploads_) {
            if (upload.arrayId == arrayId) {
                dirtyLayers.push_back(upload.layer);
            }
        }
    }

    if (dirtyLayers.empty()) return dirtyLayers;

    // Flush all pending uploads (handles all arrays)
    flushUploads(vma, device, cmdBuffer);

    // Generate mipmaps only for the dirty layers
    for (uint32_t layer : dirtyLayers) {
        generateMipmapsForLayer(arrayId, layer, cmdBuffer);
    }
