...
#ifndef ADV_POST_DOF_FOCUS_SPEED
#define ADV_POST_DOF_FOCUS_SPEED 0.2
#endif

#ifndef ADV_POST_DOF_FOCUS_HISTORY_READY
#define ADV_POST_DOF_FOCUS_HISTORY_READY false
#endif

#ifndef ADV_POST_DOF_PREVIOUS_FOCUS_BINDING
#define ADV_POST_DOF_PREVIOUS_FOCUS_BINDING 27
#endif

layout(set = 3, binding = 1) uniform sampler2D postFirstHitDepth;
layout(set = 4, binding = ADV_POST_DOF_PREVIOUS_FOCUS_BINDING) uniform sampler2D postPreviousDofFocus;
...
void main() {
    ...
    float targetFocus = ADV_POST_DOF_FOCUS_DISTANCE;
#if ADV_POST_DOF_AUTO_FOCUS != 0
    float sampleRadius = max(ADV_POST_DOF_AUTO_FOCUS_RADIUS, max(texelSize.x, texelSize.y));
    float centerDepth = sampleRawDepth(vec2(0.5));
    float depths[9];
    int validCount = 0;

    for (int i = 0; i < 9; ++i) {
        vec2 uv = clampUv(vec2(0.5) + ADV_AUTO_FOCUS_OFFSETS[i] * sampleRadius, texelSize);
    ...
    if (!ADV_POST_DOF_FOCUS_HISTORY_READY || previousFocus <= 0.0 || previousFocus >= 100000.0) {
    ...
    float focusBlend = clamp(ADV_POST_DOF_FOCUS_SPEED * mix(0.45, 2.5, distanceBoost), 0.0, 1.0);
