...
#ifndef ADV_POST_ENABLE_MOTION_BLUR
#define ADV_POST_ENABLE_MOTION_BLUR 0
#endif

#ifndef ADV_POST_MOTION_BLUR_STRENGTH
#define ADV_POST_MOTION_BLUR_STRENGTH 1.35
#endif

#ifndef ADV_POST_MOTION_BLUR_SAMPLE_COUNT
#define ADV_POST_MOTION_BLUR_SAMPLE_COUNT 10
#endif
...
void main() {
...
#if ADV_POST_ENABLE_MOTION_BLUR == 0
    outColor = vec4(baseColor, 1.0);
    return;
#else
    vec2 centerMotion = texture(postMotionVector, fragTexCoord).xy;
    float centerMotionLength = length(centerMotion);
    float blurRadiusPixels = min(centerMotionLength * ADV_POST_MOTION_BLUR_STRENGTH * 1.15, 20.0);
    ...
    for (int i = 0; i < ADV_POST_MOTION_BLUR_SAMPLE_COUNT; ++i) {
        float t = float(i + 1) / float(ADV_POST_MOTION_BLUR_SAMPLE_COUNT + 1);
