...
vec2 transmittanceUv(float r, float mu) {
    ...
    float v = clamp((r - ADV_ATMOSPHERE_RG) / (ADV_ATMOSPHERE_RT - ADV_ATMOSPHERE_RG), 0.0, 1.0);
    return vec2(u, v);
}
...
vec3 integrateSingleScattering(vec3 rayOrigin, vec3 rayDir, bool isSun) {
    float tAtm0, tAtm1;
    if (!intersectSphere(rayOrigin, rayDir, ADV_ATMOSPHERE_RT, tAtm0, tAtm1)) return vec3(0.0);
    tAtm0 = max(tAtm0, 0.0);

    float tG0, tG1;
    if (intersectSphere(rayOrigin, rayDir, ADV_ATMOSPHERE_RG, tG0, tG1)) {
        float tHitG = tG0 > 0.0 ? tG0 : tG1;
        if (tHitG > 0.0) tAtm1 = min(tAtm1, tHitG);
    }

    const int stepCount = 32;
    float dt = (tAtm1 - tAtm0) / float(stepCount);
    ...
    for (int i = 0; i < stepCount; i++) {
        ...
        float r = length(x);
        float height = r - ADV_ATMOSPHERE_RG;

        float dR = densityExp(height, ADV_ATMOSPHERE_HR);
        float dM = densityExp(height, ADV_ATMOSPHERE_HM);

        vec3 sigmaSR = ADV_ATMOSPHERE_BETA_R * dR;
        vec3 sigmaSM = ADV_ATMOSPHERE_BETA_M * dM;
        ...
        vec3 scatteredSample = viewTransmittance * (lightTransmittance * (scattering * (isSun ? ADV_SUN_RADIANCE : ADV_MOON_RADIANCE))) * dt;
