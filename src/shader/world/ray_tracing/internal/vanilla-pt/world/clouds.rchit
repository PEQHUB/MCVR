...
float cloudHitSkyLuminance(vec3 value) {
    return dot(max(value, vec3(0.0)), vec3(0.2126, 0.7152, 0.0722));
}

vec3 cloudHitFallbackSkyRadiance(vec3 rayDir) {
...
vec3 cloudHitSampleSkyRadiance(vec3 rayDir) {
    vec3 radiance = texture(skyFull, rayDir).rgb * max(skyUBO.envSky.x, 0.0);
    if (any(isnan(radiance)) || any(isinf(radiance)) ||
        (cloudHitSkyLuminance(radiance) <= 1e-5 && celestialSunDirection().y > -0.08)) {
        radiance = cloudHitFallbackSkyRadiance(rayDir);
    }
...
void main() {
    ...
    vec3 skyAmbient = cloudHitSampleSkyRadiance(normalize(viewDir));
