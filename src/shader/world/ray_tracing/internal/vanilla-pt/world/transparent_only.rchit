...
void main() {
    ...
    if (hasTexture(m0.packedData)) {
        ...
        albedo = isBlockGeometry ?
                     fetchBlockAlbedoLod(m0.textureID, uv, worldUBO.animTick, 0.0) :
                     sampleTexture(textures[nonuniformEXT(m0.textureID)], uv, 0.0, false);

        vec4 specular = isBlockGeometry ? fetchBlockSpecularLod(m0.textureID, uv, 0.0) : vec4(0.0);
        if (!isBlockGeometry) {
            int specularTextureID = mapping.entries[m0.textureID].specular;
            if (specularTextureID >= 0) {
                specular = sampleTexture(textures[nonuniformEXT(specularTextureID)], uv, 0.0, false);
            }
        }
        if (isBlockGeometry || specular.a > 0.0) {
    ...
    vec3 normal = normalize(vptWorldToObjectVector(geoNormalObj));
    ...
    mainRay.baryCoords = bary;
    mainRay.worldPos = worldPos;
    mainRay.albedoEmission =
