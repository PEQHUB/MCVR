...
float skySampleLuminance(vec3 value) {
    return dot(max(value, vec3(0.0)), vec3(0.2126, 0.7152, 0.0722));
}

bool invalidSkySample(vec3 value) {
...
vec3 sampleSkyRadiance(vec3 rayDir) {
    vec3 radiance = texture(skyFull, rayDir).rgb * max(skyUBO.envSky.x, 0.0);
    if (invalidSkySample(radiance) ||
        (skySampleLuminance(radiance) <= 1e-5 && celestialSunDirection().y > -0.08)) {
        radiance = fallbackSkyRadiance(rayDir);
    }
...
void main() {
    ...
    vec3 backgroundRadiance = mix(sampleSkyRadiance(rayDir), rainyRadiance, progress);
    ...
    if (worldUBO.skyType == 1) {
        ...
        if (sunSample.a > 1e-4) {
            ...
            if (!blocked) {
                vec3 sunRadiance = sunSample.rgb * vptPhysicalSunRadiance() * transmittance * sunSample.a;
        ...
        if (moonSample.a > 1e-4) {
            ...
            bool blocked = hitGround && (tG1 > 1e-3);
            if (!blocked) {
                vec3 moonRadiance = moonSample.rgb * vptPhysicalMoonRadiance() * max(transmittance, vec3(0.03));
