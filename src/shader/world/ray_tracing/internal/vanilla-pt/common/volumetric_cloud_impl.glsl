...
float volumetricCloudSkySampleLuminance(vec3 value) {
    return dot(max(value, vec3(0.0)), vec3(0.2126, 0.7152, 0.0722));
}

vec3 volumetricCloudFallbackSkyRadiance(vec3 rayDir) {
...
vec3 volumetricCloudSampleSkyFull(vec3 rayDir) {
    vec3 radiance = texture(skyFull, rayDir).rgb * max(skyUBO.envSky.x, 0.0);
    if (any(isnan(radiance)) || any(isinf(radiance)) ||
        (volumetricCloudSkySampleLuminance(radiance) <= 1e-5 && celestialSunDirection().y > -0.08)) {
        radiance = volumetricCloudFallbackSkyRadiance(rayDir);
    }
...
vec3 volumetricCloudSkyRadiance(vec3 sampleDir) {
    ...
    return mix(volumetricCloudSampleSkyFull(sampleDir), rainyRadiance, progress);
}
...
vec3 volumetricCloudPrimaryLightRadiance() {
    ...
    if (sunScale >= moonScale) {
        return vptPhysicalSunRadiance() * sunScale * rainAttenuation;
    }
    return vptPhysicalMoonRadiance() * moonScale * rainAttenuation;
}
...
vec3 volumetricCloudSkyFullHorizonBase(vec3 rayDir) {
    ...
    return volumetricCloudRainFilter(volumetricCloudSampleSkyFull(clampedDir), 0.95);
}
...
vec3 volumetricCloudSkyFullBelowHorizonBase(vec3 rayDir) {
    ...
    return volumetricCloudRainFilter(volumetricCloudSampleSkyFull(biasedDir), 0.70);
}
