#ifndef RARSER_FFT_WATER_GLSL
#define RARSER_FFT_WATER_GLSL
...
float sampleFftWaterCaustic(vec2 worldXZ, float gameTime, vec3 toLightDir, float waterDepth) {
    ...
    if (dot(refractedCenter, refractedCenter) <= 1e-6 ||
        dot(refractedX, refractedX) <= 1e-6 ||
        dot(refractedZ, refractedZ) <= 1e-6) {
        return 1.0;
    }
    if (refractedCenter.y >= -0.1 || refractedX.y >= -1e-3 || refractedZ.y >= -1e-3) {
        return 1.0;
    }
