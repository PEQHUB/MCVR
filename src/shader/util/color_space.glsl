...
vec4 sampleNearest(sampler2D tex, vec2 uv, int lod, bool isSRGB) {
    ...
    ivec2 coord = clamp(ivec2(floor(uv * vec2(size))), ivec2(0), max(size - ivec2(1), ivec2(0)));
    return sampleTexture(tex, coord, lod, isSRGB);
}

vec4 sampleBilinear(sampler2D tex, vec2 uv, int lod, bool isSRGB) {
    return sampleTexture(tex, uv, float(lod), isSRGB);
}
